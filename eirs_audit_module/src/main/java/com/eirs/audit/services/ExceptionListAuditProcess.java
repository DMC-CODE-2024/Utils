...
public class ExceptionListAuditProcess implements AuditProcess {
    ...
    public void fillEirsData(String operator, LocalDateTime startDate, LocalDateTime endDate) {
        ...
        try (Stream<String> stream = Files.lines(Paths.get(filepath + filename))) {
            stream.skip(1).forEach(data -> {
                eirsDataSet.put(new EirsData(data, EirlistOutputAuditConstants.EXCEPTION_LIST_NAME, EirlistOutputAuditConstants.EIRS_SOURCE, operator, filename), Boolean.TRUE);
            });
    ...
    public List<EirsData> process(Integer eirNumber, LocalDateTime startDate, LocalDateTime endDate, String operator) {
        ...
        try (Stream<String> stream = Files.lines(Paths.get(operatorFilePath + "/" + operatorEirFilename))) {
            stream.skip(1).forEach(data -> eirDataSet.put(new EirsData(data, EirlistOutputAuditConstants.EXCEPTION_LIST_NAME, EirlistOutputAuditConstants.EIR_SOURCE, operator, operatorEirFilename), Boolean.TRUE));
